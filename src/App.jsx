    ...
    <div className="app-shell">
      ...
      >
        ...
        <nav aria-label="主要导航">
          ...
          <NavItem icon={Users} label="助手团队" active={activeNav === 'agents'} onClick={() => navigate('agents')} />
  ...
  return (
    <div className="page dashboard-page" aria-busy={phase < 5}>
      <section className="page-heading">
        <div>
          <span className="eyebrow">A 股组合风险助手</span>
          <h1>两路取证，一份说得清的风险报告</h1>
          <p>新闻与公告数据分开处理，主管只合并证据，风险助手只向用户汇报。</p>
        </div>
      ...
      <section className="metrics-grid" aria-label="演示运行摘要">
        ...
        <article className="metric-card metric-with-chart">
          <RiskGauge score={scenario.riskScore} ready={reportReady} />
          <div>
            <span>演示风险分</span>
            <strong>{!reportReady ? '计算中' : scenario.riskScore === null ? '暂缓评分' : `${scenario.riskScore} / 100`}</strong>
            <small className={scenario.activeFault || quarantined ? 'neutral' : 'warning'}>
              {!reportReady ? '等待主管汇总与风险映射' : scenario.activeFault ? '故障门禁阻止不完整评分' : quarantined ? '传闻未计入组合风险' : `情景等级：${scenario.severity}`}
            </small>
          </div>
        </article>
        <Metric icon={FileCheck2} label="证据引用" value={phase < 2 ? '采集中' : `${evidenceCount} 条`} sub={phase < 2 ? '两个专项来源并行处理' : '均可追溯到本地演示样本'} type="up" />
      ...
      <section className="work-grid">
        <div className="panel holdings-panel">
          ...
          <div className="holdings-chart-row">
            <AllocationDonut holdings={holdings} centerValue="¥286k" />
          ...
          <div className="holding-list">
            {holdings.map((holding) => (
              <div className="holding" key={holding.ticker}>
                <div className="stock-icon" style={{ '--stock': holding.color }}>{holding.name.slice(0, 1)}</div>
        ...
        <div className={`panel alert-panel ${quarantined ? 'quarantine' : ''}`}>
          ...
          <div className="source-row">
            <span>演示样本</span>
            {scenario.activeFault && <span>故障：{faults.find((item) => item.id === scenario.activeFault)?.label}</span>}
          ...
          <div className="suggestion-box">
            ...
            <p>{reportReady ? scenario.recommendation : '两份证据简报提交后，主管将保留一致点、冲突与未知项；风险映射完成前不展示行动建议。'}</p>
            <div className="impact"><span>压力情景影响</span><b>{reportReady ? scenario.impact : '—'}</b></div>
          </div>
          <div className="evidence"><FileCheck2 size={17} /><p><b>{reportReady ? '依据：' : '当前阶段：'}</b>{reportReady ? scenario.rationale : '等待可引用的专项简报与主管汇总。'}</p></div>
      ...
      <section className="source-grid" aria-label="专项助手简报">
      ...
      <section className="panel workflow-panel">
        <PanelTitle title="并行取证与汇报轨迹" subtitle="专项助手独立提交同一结构；主管汇总后，风险助手才能面向用户输出" action="完整账本" onAction={() => openDrawer('ledger')} />
        <PipelineTimeline phase={phase} reviewed={reviewed} />
  ...
  return (
    <article className={`panel source-brief ${isNews ? 'news-brief' : 'data-brief'}`}>
      <div className="source-brief-heading">
        ...
        <div><span>{isNews ? '新闻专项' : '公告专项'}</span><h3>{isNews ? '新闻助手简报' : '公告助手简报'}</h3></div>
      ...
      <p>{ready ? brief.summary : '只有该助手的结构化变更被接受后，简报内容与证据编号才会显示。'}</p>
      <div className="brief-meta"><span>证据简报</span><span>证据：{ready ? brief.evidence.length : '—'} 条</span><span>可信度：{ready ? `${brief.confidence}%` : '—'}</span></div>
...
function PipelineFlow({ agents, phase, reviewed }) {
  ...
  return (
    <div className="pipeline-scroll" aria-label="新闻与公告助手并行，之后依次经过主管、风险助手和用户门禁">
...
function AgentsPage({ runtimeAgents, openDrawer }) {
  return (
    <div className="page">
      <section className="page-heading">
        <div><span className="eyebrow">团队与权限分工</span><h1>按信息来源分工，而不是重复分析</h1><p>两个专家并行取证；主管只能汇总；风险助手只能汇报，任何助手都不能交易。</p></div>
      ...
      <section className="topology-callout panel">
        <div><Newspaper size={18} /><Database size={18} /><span>两份独立证据简报</span></div>
        ...
        {runtimeAgents.map((agent, index) => {
          ...
          return (
            <article className="agent-card" key={agent.key}>
              ...
              <div className="agent-card-heading"><span>角色 0{index + 1} · {agent.stage}</span><h3>{agent.name}</h3><p>{agent.role}</p></div>
              <dl>
              ...
              <div className="permission"><LockKeyhole size={14} /><span>可写范围</span>{agent.permission.map((entry) => <code key={entry}>{entry}</code>)}</div>
      ...
      <section className="panel constitution-panel">
        <div><Sparkles size={21} /><div><h3>组织规则 v0.4</h3><p>来源分权 · 结构化变更 · 冲突不平均 · 单一匿名来源隔离 · 最终报告必须人工阅读</p></div></div>
...
function LabPage({ scenario, fault, runScenario }) {
  ...
  return (
    <div className="page">
      <section className="page-heading">
        <div><span className="eyebrow">组织对照实验</span><h1>同一证据，不同组织</h1><p>固定事件、预算和规则，只改变协作结构与故障条件；结果是演示回放，不是统计结论。</p></div>
      ...
      <section className="experiment-controls panel">
        ...
        <div className="experiment-locks"><span><LockKeyhole size={13} />预算 ¥3.50</span><span><LockKeyhole size={13} />固定演示样本</span><span><LockKeyhole size={13} />seed 20260722</span><span><LockKeyhole size={13} />相同规则版本</span></div>
      </section>

      <div className="result-caption">
        <span>上次完成回放</span>
        <b>{resultScenario.short} · {resultFault.label}</b>
        <code>单次回放 · 不代表统计结论</code>
      </div>

      <OrgScoreBars results={results} />
...
function RiskReport({ scenario, reviewed, onReview }) {
  return (
    <div className="report-view">
      <div className="report-status"><ShieldCheck size={20} /><div><span>用户风险报告 · 演示</span><h3>{scenario.userReport.status}</h3></div><code>{scenario.userReport.action}</code></div>
      ...
      <section><span className="section-kicker">主管汇总</span><p>{scenario.synthesis.agreement}</p>{scenario.synthesis.conflicts.map((item) => <div className="warning-line" key={item}><AlertTriangle size={14} />{item}</div>)}</section>
...
function LedgerView({ ledger }) {
  return (
    <div className="ledger-list">
      <p className="drawer-intro">每个助手只能提交自己职责范围内的变更记录；下游通过编号引用上游证据，无法静默覆盖。</p>
      {ledger.map((patch, index) => (
        <article key={patch.id}>
          <div className="ledger-index">0{index + 1}</div>
          <div><span>{patch.author}</span><h3>{patch.id}</h3><code>{patch.target}</code><p>{patch.summary}</p><small>引用：{patch.evidence.join(' · ') || '无'}</small></div>
...
function EvidenceView({ scenario }) {
  return (
    <div className="evidence-view">
      <p className="drawer-intro">两个来源助手使用同一最小输出合同，但不共享结论；主管收到两份简报后才开始汇总。</p>
      {Object.entries(scenario.evidenceBriefs).map(([type, brief]) => (
        <section className="evidence-brief" key={brief.id}>
          ...
          <p>{brief.summary}</p>
          <div className="schema-row plain-meta">
            <span>来源类型：{brief.sourceClass}</span>
            <span>提供方：{brief.provider}</span>
            <span>更新时间：{brief.asOf}</span>
            <span>可信度：{brief.confidence}%</span>
          </div>
          <h4>主要发现</h4><ul>{brief.findings.map((finding) => <li key={finding}>{finding}</li>)}</ul>
          <h4>证据</h4><div className="evidence-items">{brief.evidence.map((item) => <div key={item.id}><span className={item.verified ? 'verified-dot' : 'unverified-dot'} /><div><b>{item.label}</b><small>{item.id} · {item.tier} · {item.freshness}</small><code>演示样本编号：{item.id}</code><p>{item.note}</p></div></div>)}</div>
...
function RulesView() {
  const rules = [
    ['G-01', '来源分权', '新闻助手与公告助手不能替对方修改简报。'],
    ['G-02', '结构化写入', '所有状态变化必须通过包含证据编号的变更记录。'],
    ['G-03', '冲突保留', '主管不得把相互冲突的事实简单平均或静默删除。'],
    ['G-04', '匿名来源隔离', '单一匿名来源默认进入隔离。'],
    ['G-05', '人类门禁', '报告只能被标记为已阅读，系统无券商连接与交易工具。'],
    ['G-06', '演示真实性', '演示、延迟和实时数据必须在每条记录上明确标记。'],
  ];
...
function NotificationsView({ scenario, phase }) {
  return <div className="notification-list"><article><AlertTriangle size={18} /><div><span>演示风险报告</span><h3>{scenario.short}</h3><p>{phase >= 5 ? '固定回放已生成用户报告；需要人工阅读。' : '专项助手正在处理；最终报告尚未生成。'}</p></div></article><article><FlaskConical size={18} /><div><span>数据状态</span><h3>所有输入均为演示样本</h3><p>当前页面未连接巨潮行情、券商或真实账户。</p></div></article></div>;
}

function HoldingsDetail() {
  return <div className="holdings-detail"><p className="drawer-intro">以下金额、收益与仓位全部为虚构演示数据。</p>{holdings.map((holding) => <article key={holding.ticker}><div className="stock-icon" style={{ '--stock': holding.color }}>{holding.name[0]}</div><div><h3>{holding.name}</h3><span>{holding.market} · {holding.ticker}</span></div><dl><div><dt>仓位</dt><dd>{holding.allocation}%</dd></div><div><dt>演示收益</dt><dd className={holding.pnl > 0 ? 'positive' : 'negative'}>{holding.pnl > 0 ? '+' : ''}{holding.pnl}%</dd></div><div><dt>规则风险</dt><dd>{holding.risk}</dd></div></dl></article>)}</div>;
}
