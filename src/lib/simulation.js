...
export const PIPELINE_STAGES = [
  { phase: 0, label: '事件入队' },
  { phase: 1, label: '专项 Agent 并行分析' },
  { phase: 2, label: '结构化简报提交' },
  { phase: 3, label: '主管综合与冲突检查' },
  { phase: 4, label: '风险映射' },
    ...
    return materializeScenarioRun({
      ...
      evidenceBriefs: {
        ...
        news: {
          ...
          confidence: 0,
          headline: '故障注入：新闻 Agent 在截止时间前未返回',
          summary: '新闻适配器被模拟为不可用；系统记录降级状态，而不是补写或猜测新闻事实。',
          findings: ['未取得新闻内容', '未执行自动重试风暴', '公告/数据链路继续独立运行'],
          evidence: [{ id: 'SIM-RUNTIME-TIMEOUT', label: '模拟运行时超时事件', locator: 'fixture://runtime/SIM-RUNTIME-TIMEOUT', tier: '系统事件', verified: true, freshness: 'T+0', note: '故障种子 news-timeout；不是金融证据' }],
          gaps: ['全部新闻侧事实与交叉核验'],
        },
        filing: {
      ...
      evidenceBriefs: {
        ...
        news: {
          ...
          evidence: [
            ...
            { id: 'SIM-FAULT-CONFLICT', label: '模拟证据冲突标记', locator: 'fixture://runtime/SIM-FAULT-CONFLICT', tier: '实验控制', verified: true, freshness: 'T+0', note: '故障种子 conflicting-evidence；不是金融证据' },
          ],
          gaps: ['可裁决冲突的新增一级来源'],
        },
        filing: {
  ...
  const ledger = [
    {
      ...
      summary: phase >= 2 ? `status: ${newsStatus} · rule_score: ${scenario.evidenceBriefs.news.confidence}%` : '等待 EvidenceBrief 提交',
    ...
    },
    {
      ...
      summary: phase >= 2 ? `status: ${scenario.evidenceBriefs.filing.status} · rule_score: ${scenario.evidenceBriefs.filing.confidence}%` : '等待 EvidenceBrief 提交',
    ...
    },
    {
      ...
      summary: phase >= 3 ? `decision: ${scenario.synthesis.decision} · conflicts: ${scenario.synthesis.conflicts.length}` : '等待两份 EvidenceBrief 汇合',
    ...
    },
    {
      ...
      summary: phase >= 5 ? `action: ${scenario.userReport.action} · human_gate: REQUIRED` : '等待 SupervisorSynthesis 与风险映射',
